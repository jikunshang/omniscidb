...
int main(int argc, char* argv[]) {
  ...
  try {
    int dbe_instance = 4;
    std::vector<std::shared_ptr<DBEngine> > engines(dbe_instance);
    for (int i = 0; i < dbe_instance; i++) {
      auto opt_str = base_path + std::to_string(i) + " --calcite-port " +
                     std::to_string(calcite_port + i);
      engines[i] = DBEngine::create(opt_str);
    }
    ...
    //     threads[i] = std::thread([&]{dbe->executeDML("select * from test");});
    //   }
    //   for(int i=0; i < thread_num; i++) {
    //     threads[i].join();
    //   }
